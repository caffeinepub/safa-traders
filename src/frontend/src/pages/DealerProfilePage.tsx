...
import DealerProfileSetup from '../components/DealerProfileSetup';
...
export default function DealerProfilePage() {
  ...
  useEffect(() => {
    if (isAuthenticated && isFetched) {
      if (userProfile === null) {
        setShowSetup(true);
      } else if (userProfile && userProfile.profileType === ProfileType.company) {
        // User has a company profile, redirect or show message
        setShowSetup(false);
      } else {
        setShowSetup(false);
      }
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center px-4 py-16">
          <Card className="max-w-md w-full">
            <CardHeader>
              <CardTitle className="text-2xl">Dealer Login Required</CardTitle>
              <CardDescription>
                Please log in to access your dealer profile
              </CardDescription>
            </CardHeader>
            <CardContent>
              <button
  ...
  if (userProfile && userProfile.profileType === ProfileType.company) {
    return (
      <div className="min-h-screen flex flex-col">
        <Header />
        <main className="flex-1 flex items-center justify-center px-4 py-16">
          <Card className="max-w-md w-full">
            <CardHeader>
              ...
              <CardDescription>
                You are logged in with a company profile. Please use the company profile page instead.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <a
      ...
      <main className="flex-1 px-4 py-8">
        <div className="container mx-auto max-w-4xl">
          {showSetup || userProfile === null || !userProfile ? (
            <DealerProfileSetup onComplete={() => setShowSetup(false)} />
          ) : (
            <DealerProfileView profile={userProfile} />
          )}
        </div>
      </main>
      <Footer />
